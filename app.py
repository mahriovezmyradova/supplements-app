...
def patient_inputs():
    ...
    if (
        ...
        result = load_patient_data(patient_name_input)
        if result[0]:
            patient_data, nem_prescriptions, therapieplan_data, ernaehrung_data, infusion_data = result
    ...
    if isinstance(default_geburtsdatum, str):
        try:
            default_geburtsdatum = date.fromisoformat(default_geburtsdatum)
        except:
            default_geburtsdatum = date.today()
    ...
    if isinstance(default_therapiebeginn, str):
        try:
            default_therapiebeginn = date.fromisoformat(default_therapiebeginn)
        except:
            default_therapiebeginn = date.today()
    ...
    with c6:
        dauer = st.selectbox(
            "Dauer (Monate)",
            list(range(1, 13)),
            index=default_dauer_value - 1 if isinstance(default_dauer_value, int) else 5,
        )
...
def main():
    ...
    if 'nem_form_data' not in st.session_state:
        st.session_state.nem_form_data = {}
    ...
    with st.sidebar:
        ...
        if st.button("NEM Session State anzeigen"):
            st.write("### NEM Prescriptions in Session")
            st.write(f"Anzahl: {len(st.session_state.get('nem_prescriptions', []))}")
            if st.session_state.get('nem_prescriptions'):
                st.json(st.session_state.nem_prescriptions[:3])
            else:
                st.warning("Keine NEM-Daten in session_state!")
        
        if st.button("Category States anzeigen"):
            st.write("### Category States")
            st.json(st.session_state.get('category_states', {}))
        
        if st.button("Session State zurücksetzen (nur NEM)"):
            st.session_state.nem_prescriptions = []
            st.success("NEM Session State zurückgesetzt!")
    ...
    with tabs[1]:
        st.header("Nahrungsergänzungsmittel (NEM)")
        
        # Create a unique key for this session
        if 'nem_form_data' not in st.session_state:
            st.session_state.nem_form_data = {}
        
        # Create a container for the NEM form
        with st.form("nem_form", clear_on_submit=False):
            ...
            st.markdown("""
                ...
                .empty-rows-header {
                    ...
                    border-radius: 4px;
                    border-left: 4px solid rgb(38, 96, 65);
                    font-weight: bold;
                }
                </style>
            """, unsafe_allow_html=True)
            ...
            st.markdown('<div class="empty-rows-header">➕ Freie Einträge für nicht gelistete Supplemente</div>', unsafe_allow_html=True)
            
            for i in range(5):
                cols = st.columns([2.2, 0.9, 1.2, 1, 0.7, 0.7, 0.7, 0.7, 0.7, 2.3])
                
                custom_name = cols[0].text_input(
                    "", key=f"custom_name_{i}", placeholder="Supplement eintragen...",
                    label_visibility="collapsed"
                )
                
                gesamt_dosierung_val = cols[1].selectbox(
                    "", gesamt_dosierung_options, index=0,
                    key=f"custom_gesamt_{i}", label_visibility="collapsed"
                )
                
                selected_form = cols[2].selectbox(
                    "", ["Kapseln", "Lösung", "Tabletten", "Pulver", "Tropfen", "Sachet", "Öl", "Spray", "Creme", "Gel", "Flüssig", "Tee", "Pflaster", "Andere:"], 
                    index=0,
                    key=f"custom_form_{i}", label_visibility="collapsed"
                )
                
                custom_form = ""
                if selected_form == "Andere:":
                    custom_form = cols[2].text_input(
                        "", key=f"custom_form_text_{i}", placeholder="z. B. Pulver",
                        label_visibility="collapsed"
                    )
                
                current_form = st.session_state.get(f"custom_form_{i}", selected_form)
                pro_einnahme_options = get_pro_einnahme_options(current_form)
                
                pro_einnahme_val = cols[3].selectbox(
                    "", pro_einnahme_options, index=0,
                    key=f"custom_pro_{i}", label_visibility="collapsed"
                )
                
                dose_options = ["", "1", "2", "3", "4", "5"]
                
                nue_val = cols[4].selectbox("", dose_options, index=0,
                                        key=f"custom_nue_{i}", label_visibility="collapsed")
                morg_val = cols[5].selectbox("", dose_options, index=0,
                                        key=f"custom_morg_{i}", label_visibility="collapsed")
                mitt_val = cols[6].selectbox("", dose_options, index=0,
                                        key=f"custom_mitt_{i}", label_visibility="collapsed")
                abend_val = cols[7].selectbox("", dose_options, index=0,
                                            key=f"custom_abend_{i}", label_visibility="collapsed")
                nacht_val = cols[8].selectbox("", dose_options, index=0,
                                            key=f"custom_nacht_{i}", label_visibility="collapsed")
                
                comment = cols[9].text_input(
                    "", key=f"custom_comment_{i}", placeholder="Kommentar",
                    label_visibility="collapsed"
                )
            ...
            # Add a button to save NEM data to session state
            st.markdown("---")
            col1, col2, col3 = st.columns(3)
            
            with col1:
                save_nem_button = st.form_submit_button("💾 NEM-Daten speichern", use_container_width=True)
            
            with col2:
                pdf_button = st.form_submit_button("📄 PDF generieren", use_container_width=True)
            
            with col3:
                show_data_button = st.form_submit_button("🔍 Daten anzeigen", use_container_width=True)
            
            # Handle button clicks
            if save_nem_button or pdf_button or show_data_button:
                # Update session state with current form data
                st.session_state.nem_prescriptions = all_supplements_data
                st.session_state.nem_form_data = {
                    "count": len(all_supplements_data),
                    "data": all_supplements_data[:2] if all_supplements_data else []  # Store first 2 for preview
                }
                
                if save_nem_button:
                    st.success(f"✅ {len(all_supplements_data)} NEM-Einträge wurden gespeichert!")
                
                elif pdf_button:
                    if all_supplements_data:
                        pdf_bytes = generate_pdf(patient, all_supplements_data, "NEM")
                        ...
                        st.success(f"✅ PDF wird generiert mit {len(all_supplements_data)} Einträgen!")
                    else:
                        st.warning("⚠️ Keine NEM-Daten zum Generieren eines PDFs vorhanden!")
                
                elif show_data_button:
                    st.info(f"📊 Aktuell {len(all_supplements_data)} NEM-Einträge in diesem Formular")
                    if all_supplements_data:
                        st.json(all_supplements_data[:3])  # Show first 3 entries
...
# Handle save button
    if save_button:
        if not patient["patient"]:
            ...
            print(f"NEM prescriptions count: {len(st.session_state.nem_prescriptions)}")
            ...
            with st.expander("📋 Vorschau der zu speichernden Daten"):
                st.write(f"**NEM-Einträge:** {len(nem_prescriptions)}")
                if nem_prescriptions:
                    st.json(nem_prescriptions[:3])
                else:
                    st.warning("Keine NEM-Daten vorhanden!")
