...
# Enhanced CSS - cleaner spacing, larger readable font, refined aesthetics
st.markdown("""
<style>
...
.header-title { flex: 2; text-align: center; margin: 0; }
.header-address { flex: 1; text-align: right; }
.header-address div {
...
.stButton > button {
    ...
    box-shadow: 0 2px 6px rgba(38,96,65,0.25) !important;
    font-family: 'DM Sans', sans-serif !important;
}
.stButton > button:hover {
    background-color: rgb(28, 76, 50) !important;
    box-shadow: 0 4px 12px rgba(38,96,65,0.35) !important;
...
.stTabs [aria-selected="true"] {
    ...
    border-bottom: 3px solid rgb(38, 96, 65) !important;
...
[data-testid="stDateInput"] > div > div > input {
    ...
    font-size: 16px !important;
    border-radius: 8px !important;
    border: 1.5px solid #d4dae3 !important;
...
.stTextArea > div > div > textarea:focus {
    ...
    box-shadow: 0 0 0 3px rgba(38,96,65,0.1) !important;
}

/* Bigger textarea for diagnosen and kommentar */
textarea {
    font-size: 16px !important;
...
[data-testid="stCheckbox"] input[type="checkbox"] {
    accent-color: rgb(38, 96, 65) !important;
}
...
/* ===== MULTI-SELECT TAGS ===== */
[data-baseweb="tag"] {
    background-color: rgba(38,96,65,0.1) !important;
...
.section-subheader {
    ...
    color: rgb(38, 96, 65) !important;
    border-bottom: 2px solid rgba(38,96,65,0.2) !important;
...
.streamlit-expanderHeader {
    ...
    color: rgb(38, 96, 65) !important;
    font-family: 'DM Sans', sans-serif !important;
    border: 1.5px solid rgba(38,96,65,0.15) !important;
...
.streamlit-expanderContent {
    ...
    border: 1.5px solid rgba(38,96,65,0.1) !important;
...
.info-icon {
    ...
    height: 18px;
    border-radius: 50%;
    background-color: rgba(38,96,65,0.15);
    color: rgb(38, 96, 65);
    font-size: 11px;
    font-weight: bold;
    cursor: help;
...
/* ===== ELEMENT SPACING — tighter but readable ===== */
...
with col3:
    ...
    st.markdown("""
    <div style="font-size:14px; line-height:1.8; color:#555; text-align:right;">
    Clausewitzstr. 2<br>
...
def therapy_progress_bar(therapiebeginn, dauer_monate):
    ...
    html = f"""
    <div style="margin: 12px 0 8px 0; padding: 16px 20px; background: #f0f4f0; border-radius: 12px; border: 1.5px solid rgba(38,96,65,0.15);">
        ...
        <div style="position: relative; height: 36px; background: #dde8e2; border-radius: 18px; overflow: visible;">
            ...
            <div style="position: absolute; top: -2px; left: calc({progress}% - 20px);">
                <div style="background: white; border-radius: 50%; width: 40px; height: 40px;
                            border: 2.5px solid rgb(38, 96, 65); display: flex; align-items: center;
                            justify-content: center; box-shadow: 0 3px 8px rgba(38,96,65,0.25);">
...
def patient_inputs():
    ...
    with c1:
        geburtsdatum = st.date_input(
            "Geburtsdatum", value=default_geburtsdatum,
            min_value=date(1900, 1, 1), max_value=date.today(),
            format="DD.MM.YYYY", key="geburtsdatum_input"
        )
    with c2:
        geschlecht = st.radio(
    ...
    with c5:
        therapiebeginn = st.date_input(
            "Therapiebeginn", value=default_therapiebeginn,
            format="DD.MM.YYYY", key="therapiebeginn_input"
        )
    with c6:
        dauer = st.selectbox(
            "Dauer (Monate)", list(range(1, 13)),
            index=default_dauer_value - 1 if 1 <= default_dauer_value <= 12 else 5,
            key="dauer_input"
        )
    with c7:
        tw_besprochen = st.radio(
            "TW besprochen?", ["Ja", "Nein"], horizontal=True,
            index=0 if default_tw_besprochen == "Ja" else 1,
            key="tw_besprochen_input"
        )

    # *** CHANGED: Bekannte Allergie → Bekannte Allergien, with more space ***
    bekannte_allergie = st.text_area(
        "Bekannte Allergien",
        value=default_allergie,
        height=90,
        placeholder="Bekannte Allergien eintragen...",
        key="allergie_input"
    )

    # *** More space for Diagnosen ***
    ...
    # *** More space for Kommentar ***
    kontrolltermin_kommentar = st.text_area(
        "Kommentar:",
        value=default_kontrolltermin_kommentar,
        height=100,
        placeholder="Kommentar zu Kontrollterminen...",
        key="kontrolltermin_kommentar_input"
    )
...
def therapie_row(label, key_prefix, therapiebeginn):
    checkbox_key = f"th_{key_prefix}_cb"
    start_key = f"th_{key_prefix}_start"
    end_key = f"th_{key_prefix}_end"
    freq_key = f"th_{key_prefix}_freq"
    ...
    with cols[0]:
        ...
        with row[0]:
            value = st.checkbox(
                "",
                value=st.session_state.therapieplan_data.get(key_prefix, False),
                key=checkbox_key,
                label_visibility="collapsed"
            )
        with row[1]:
            st.markdown(
                f"<div style='margin-top:6px;font-size:14px;'>{label}</div>",
                unsafe_allow_html=True
            )

    # --- MIDDLE: AUTO DATE RANGE ---
    with cols[1]:
        start_default = st.session_state.therapieplan_data.get(start_key, therapiebeginn)
        end_default = st.session_state.therapieplan_data.get(
            end_key,
            start_default + timedelta(days=6)
        )
    ...
    # --- RIGHT: FREQUENCY ---
    with cols[2]:
        freq_options = ["", "1x/Woche", "2x/Woche", "3x/Woche", "4x/Woche"]
        freq = st.selectbox(
            "",
            freq_options,
            index=freq_options.index(
                st.session_state.therapieplan_data.get(freq_key, "")
            ) if st.session_state.therapieplan_data.get(freq_key, "") in freq_options else 0,
            key=freq_key,
            label_visibility="collapsed"
        )

    return value, start, end, freq
...
def main():
    ...
    # =========================================================
    # TAB 0: THERAPIEPLAN — now with collapsible major sections
    # =========================================================
    with tabs[0]:

        # ---- SECTION 1: Diagnostik & Überprüfung (collapsible) ----
        with st.expander("Diagnostik & Überprüfung", expanded=st.session_state.therapieplan_data.get("_sec_diagnostik_open", True)):
            ...
            if zaehne:
                zaehne_zu_pruefen = st.text_input(
                    "Zähne zu überprüfen (OPG/DVT):",
            ...
            analyse_bewegungsapparat = st.checkbox(
        ...
        # ---- SECTION 2: Haupttherapien (collapsible) - includes all main therapies ----
        with st.expander("Haupttherapien", expanded=st.session_state.therapieplan_data.get("_sec_haupttherapien_open", False)):
            
            # Subsection: Darm & Entgiftung (includes everything)
            st.markdown('<div class="section-subheader">Darm & Entgiftung</div>', unsafe_allow_html=True)
            
            darm_biofilm = st.checkbox(
            ...
            darmsanierung = st.checkbox(
                "Darmsanierung nach Paracelsus Klinik (Rezept von Praxis)",
                value=st.session_state.therapieplan_data.get("darmsanierung", False),
                key="darmsanierung_checkbox"
            )
            if darmsanierung:
                darmsanierung_dauer = st.multiselect(
                    "Darmsanierung Dauer:", ["4 Wo", "6 Wo", "8 Wo"],
        ...
        with st.expander("Biologische & Komplementäre Therapien", expanded=st.session_state.therapieplan_data.get("_sec_bio_open", False)):
            ...
            with left:
                ...
                <div style="margin-bottom: 2px;">
                    <div class="section-subheader" style="margin-bottom: 2px;">Hypnosetherapie</div>
                </div>
                """, unsafe_allow_html=True)
                ...
                # VERY TIGHT doctor checkboxes
                c1, c2, c3 = st.columns([1, 1, 1])
                with c1:
                    hypnose_noreen = st.checkbox("Noreen", key="hypnose_noreen_checkbox")
                with c2:
                    hypnose_martin = st.checkbox("Martin", key="hypnose_martin_checkbox")
                with c3:
                    hypnose_miro = st.checkbox("Miro", key="hypnose_miro_checkbox")

                # Inline small input
                col1, col2 = st.columns([2, 1])
                with col1:
                    st.markdown("Hypnose Typ:")
                with col2:
                    hypnose_typ = st.text_input(
                        "",
                        key="hypnose_typ_input",
                        placeholder="",
                        label_visibility="collapsed"
                    )

                yager = st.checkbox("Yagertherapie", key="yager_checkbox")
                energie_behandlungen = st.checkbox("Energiebehandlungen bei Marie", key="energie_behandlungen_checkbox")


                # -------------------- Ernährung & Bewegung --------------------
                st.markdown('<div class="section-subheader">Ernährung & Bewegung</div>', unsafe_allow_html=True)

                def tight_row(label, key_cb, key_input):
                    c1, c2 = st.columns([3, 1])  # compact input
                    with c1:
                        val = st.checkbox(label, key=key_cb)
                    with c2:
                        txt = st.text_input("", key=key_input, placeholder="", label_visibility="collapsed")
                    return val, txt
                ...
                # Sub-options (slight indent but still left compact)
                st.markdown('<div style="margin-left: 10px;">', unsafe_allow_html=True)

                fasten, fasten_comment = tight_row("Intermittierendes Fasten", "fasten", "fasten_comment")
                krebsdiaet, krebsdiaet_comment = tight_row("Krebs Diät", "krebsdiaet", "krebsdiaet_comment")
                ketogene, ketogene_comment = tight_row("Ketogene Ernährung", "ketogene", "ketogene_comment")
                basisch, basisch_comment = tight_row("Basische Ernährung", "basisch", "basisch_comment")

                st.markdown('</div>', unsafe_allow_html=True)

                # NO extra comment fields here (clean + compact)
                naehrstoff_ausgleich = st.text_input(
                    "Nährstoffmängel ausgleichen:",
                    key="naehrstoff_ausgleich_input"
                )

                therapie_sonstiges = st.text_input(
                    "Sonstiges:",
                ...
                st.markdown("""
                <div style="margin-bottom: 2px;">
                    <div class="section-subheader" style="margin-bottom: 2px;">Ästhetische Behandlung</div>
                </div>
                """, unsafe_allow_html=True)

                c1, c2 = st.columns([3, 1])
                with c1:
                    aethetisch = st.checkbox("Ästhetische Behandlung", key="aethetisch_checkbox")
                with c2:
                    aethetisch_comment = st.text_input("", key="aethetisch_comment_input", placeholder="", label_visibility="collapsed")

                if aethetisch:
                    st.markdown('<span style="font-size: 13px;">Behandlungsart:</span>', unsafe_allow_html=True)

                    # CRITICAL FIX: tightly packed checkboxes LEFT
                    c1, c2, c3, c4 = st.columns([1, 1, 1, 1])
                    with c1:
                        aethetisch_botox = st.checkbox("Botox", key="aethetisch_botox_checkbox")
                    with c2:
                        aethetisch_prp = st.checkbox("PRP", key="aethetisch_prp_checkbox")
                    with c3:
                        aethetisch_faeden = st.checkbox("Fäden", key="aethetisch_faeden_checkbox")
                    with c4:
                        aethetisch_hyaloron = st.checkbox("Hyaloron", key="aethetisch_hyaloron_checkbox")
                else:
                    aethetisch_botox = aethetisch_prp = aethetisch_faeden = aethetisch_hyaloron = False


            # RIGHT SIDE RESERVED (empty for future table)
            with right:
                st.empty()


        # ---- SECTION 4: Gespräche (collapsible) ----
        with st.expander("Gespräche", expanded=st.session_state.therapieplan_data.get("_sec_gespraeche_open", False)):
            zwischengespraech_4 = st.checkbox(
                "Zwischengespräch nach 4 Wochen (1/2h)",
                value=st.session_state.therapieplan_data.get("zwischengespraech_4", False),
                key="zwischengespraech_4_checkbox"
            )
            zwischengespraech_8 = st.checkbox(
                "Zwischengespräch nach weiteren 8 Wochen (1/2h)",
                value=st.session_state.therapieplan_data.get("zwischengespraech_8", False),
                key="zwischengespraech_8_checkbox"
            )

        # Update session state for Therapieplan
        st.session_state.therapieplan_data = {
            # Diagnostik
            "zaehne": zaehne,
            ...
            # Haupttherapien - Darm & Entgiftung
            ...
            # Haupttherapien - Ausleitung & Infektionen
            ...
            # Biologische & Komplementäre Therapien
            ...
            "hypnose": hypnose,
            "hypnose_noreen": hypnose_noreen,
            "hypnose_martin": hypnose_martin,
            "hypnose_miro": hypnose_miro,
            "hypnose_typ": hypnose_typ,
            "yager": yager,
            ...
            "aethetisch_botox": aethetisch_botox,
            "aethetisch_prp": aethetisch_prp,
            "aethetisch_faeden": aethetisch_faeden,
            "aethetisch_hyaloron": aethetisch_hyaloron,
            "aethetisch_comment": aethetisch_comment,
    ...
    with tabs[2]:
        ...
        st.markdown("<hr style='margin-top: 4px; margin-bottom: 16px; border-color: rgb(38, 96, 65); border-width: 2px;'>", unsafe_allow_html=True)
        ...
        def infusion_row(label, key_prefix, tooltip, default_checked=False, default_weeks="", default_freq=""):
                ...
                with row[1]:
                    st.markdown(
                        f"""<div style="display:flex;align-items:center;gap:4px;margin-top:8px;">
                            <span style="font-size:15px;line-height:1;font-family:'DM Sans',sans-serif;">{label}</span>
            ...
            with cols[1]:
                ...
                weeks_value = st.selectbox(
                    ...
                    key=weeks_key, label_visibility="collapsed", placeholder="Wochen"
                )
            with cols[2]:
                ...
                freq_value = st.selectbox(
                    ...
                    key=freq_key, label_visibility="collapsed", placeholder="Häufigkeit"
                )
            ...
            with cols[0]:
                ...
                with label_col:
                    ...
                    <div style="display: flex; align-items: center; gap: 6px;">
                        <span style="font-size: 15px; white-space: nowrap; font-family: 'DM Sans', sans-serif;">{label}</span>
                        <span class="info-icon" data-tooltip="{tooltip}">ⓘ</span>
                        <input id="{input_id}" type="text" placeholder="ml" value="{current_procain}"
                            style="width: 50px; padding: 4px 8px; font-size: 14px; border-radius: 6px; border:1.5px solid #d4dae3; font-family: 'DM Sans', sans-serif;">
        ...
        revita_immune, revita_immune_weeks, revita_immune_freq = infusion_row("RevitaImmune", "revita_immune", "Vitamin C, Zink, Selen, Magnesium, B-Vitamine")
        revita_immune_plus, revita_immune_plus_weeks, revita_immune_plus_freq = infusion_row("RevitaImmunePlus", "revita_immune_plus", "Hochdosiert: Vitamin C, Zink, Selen, Magnesium, B-Vitamine, Glutathion")
        revita_heal, revita_heal_weeks, revita_heal_freq = infusion_row("Revita Heal (2x)", "revita_heal", "Vitamin C, Zink, Arginin, Glutamin, B-Vitamine, Magnesium")
        revita_bludder, revita_bludder_weeks, revita_bludder_freq = infusion_row("RevitaBludder", "revita_bludder", "Eisen, Vitamin B12, Folsäure, Vitamin C")
        revita_ferro, revita_ferro_weeks, revita_ferro_freq = infusion_row("RevitaFerro", "revita_ferro", "Ferinject (Eisen), Vitamin C")
        revita_energy, revita_energy_weeks, revita_energy_freq = infusion_row("RevitaEnergyBoost", "revita_energy", "Magnesium, B-Vitamine, Vitamin C, Coenzym Q10")
        revita_focus, revita_focus_weeks, revita_focus_freq = infusion_row("RevitaFocus", "revita_focus", "Magnesium, B-Vitamine, Vitamin C, Zink, Alpha-Liponsäure")
        revita_nad, revita_nad_weeks, revita_nad_freq = infusion_row("RevitaNAD+", "revita_nad", "NAD+ 500mg (oder 125mg), Magnesium, B-Vitamine")
        revita_relax, revita_relax_weeks, revita_relax_freq = infusion_row("RevitaRelax", "revita_relax", "Magnesium, B-Vitamine, Vitamin C, Calcium")
        revita_fit, revita_fit_weeks, revita_fit_freq = infusion_row("RevitaFit", "revita_fit", "Magnesium, B-Vitamine, Vitamin C, Aminosäuren, Coenzym Q10")
        revita_hangover, revita_hangover_weeks, revita_hangover_freq = infusion_row("RevitaHangover", "revita_hangover", "Elektrolyte, Vitamin C, B-Vitamine, Magnesium, Glutathion")
        revita_beauty, revita_beauty_weeks, revita_beauty_freq = infusion_row("RevitaBeauty", "revita_beauty", "Vitamin C, Biotin, Zink, Selen, B-Vitamine")
        revita_antiaging, revita_antiaging_weeks, revita_antiaging_freq = infusion_row("RevitaAnti-Aging", "revita_antiaging", "Glutathion, Vitamin C, Alpha-Liponsäure, Selen, Zink")
        revita_detox, revita_detox_weeks, revita_detox_freq = infusion_row("RevitaDetox", "revita_detox", "Glutathion, Vitamin C, Magnesium, B-Vitamine")
        revita_chelate, revita_chelate_weeks, revita_chelate_freq = infusion_row("RevitaChelate", "revita_chelate", "EDTA, DMSA, Vitamin C, Magnesium, Zink")
        revita_liver, revita_liver_weeks, revita_liver_freq = infusion_row("RevitaLiver", "revita_liver", "Glutathion, Vitamin C, B-Vitamine, Magnesium, Mariendistel-Extrakt")
        revita_leakygut, revita_leakygut_weeks, revita_leakygut_freq = infusion_row("RevitaLeaky-gut", "revita_leakygut", "Glutamin, Zink, Vitamin C, B-Vitamine, Magnesium")
        revita_infection, revita_infection_weeks, revita_infection_freq = infusion_row("RevitaInfection", "revita_infection", "Vitamin C, Zink, Selen, Magnesium, B-Vitamine, Glutathion")
        revita_joint, revita_joint_weeks, revita_joint_freq = infusion_row("RevitaJoint", "revita_joint", "Vitamin C, Magnesium, Zink, Mangan, B-Vitamine")
        ...
        mito_energy, mito_energy_weeks, mito_energy_freq = infusion_row("Mito-Energy Behandlung (Mito-Gerät, Wirkbooster)", "std_mito_energy", "Mito-Energy Behandlung mit Wirkbooster")
        oxyvenierung, oxyvenierung_weeks, oxyvenierung_freq = infusion_row("Oxyvenierung (10–40 ml, 10er Serie)", "std_oxyvenierung", "Oxyvenierung (10–40 ml, 10er Serie)")
        schwermetalltest, schwermetalltest_weeks, schwermetalltest_freq = infusion_row("Schwermetalltest mit DMSA und Ca EDTA", "std_schwermetalltest", "Test mit DMSA und Ca EDTA")
        procain_basen, procain_basen_weeks, procain_basen_freq, procain_2percent = procain_row("Procain Baseninfusion mit Magnesium", "std_procain_basen", "Procain Baseninfusion mit Magnesium")
        artemisinin, artemisinin_weeks, artemisinin_freq = infusion_row("Artemisinin Infusion mit 2x Lysin", "std_artemisinin", "Artemisinin Infusion mit 2x Lysin")
        perioperative, perioperative_weeks, perioperative_freq = infusion_row("Perioperative Infusion (3 Infusionen)", "std_perioperative", "Perioperative Infusion (3 Infusionen)")
        detox_standard, detox_standard_weeks, detox_standard_freq = infusion_row("Detox-Infusion Standard", "std_detox_standard", "Detox-Infusion Standard")
        detox_maxi, detox_maxi_weeks, detox_maxi_freq = infusion_row("Detox-Infusion Maxi", "std_detox_maxi", "Detox-Infusion Maxi")
        aufbauinfusion, aufbauinfusion_weeks, aufbauinfusion_freq = infusion_row("Aufbauinfusion nach Detox", "std_aufbauinfusion", "Aufbauinfusion nach Detox")
        anti_aging, anti_aging_weeks, anti_aging_freq = infusion_row("Anti Aging Infusion komplett", "std_anti_aging", "Anti Aging Infusion komplett")
        nerven_aufbau, nerven_aufbau_weeks, nerven_aufbau_freq = infusion_row("Nerven Aufbau Infusion", "std_nerven_aufbau", "Nerven Aufbau Infusion")
        leberentgiftung, leberentgiftung_weeks, leberentgiftung_freq = infusion_row("Leberentgiftungsinfusion", "std_leberentgiftung", "Leberentgiftungsinfusion")
        anti_oxidantien, anti_oxidantien_weeks, anti_oxidantien_freq = infusion_row("Anti-Oxidantien Infusion", "std_anti_oxidantien", "Anti-Oxidantien Infusion")
        aminoinfusion, aminoinfusion_weeks, aminoinfusion_freq = infusion_row("Aminoinfusion leaky gut (5–10)", "std_aminoinfusion", "Aminoinfusion leaky gut (5–10)")
        relax_infusion, relax_infusion_weeks, relax_infusion_freq = infusion_row("Relax Infusion", "std_relax_infusion", "Relax Infusion")
        ...
        infektions_infusion = st.text_input(
            ...
            key="infektions_infusion_input", placeholder="Anzahl / ml"
        )
        immun_booster = st.selectbox(
            "Immun-Boosterung Typ", ["", "Typ 1", "Typ 2", "Typ 3"],
        ...
        vitamin_c, vitamin_c_weeks, vitamin_c_freq = infusion_row("Hochdosis Vitamin C (g)", "single_vitamin_c", "Hochdosiertes Vitamin C")
        vitamin_b_komplex, vitamin_b_komplex_weeks, vitamin_b_komplex_freq = infusion_row("Vit. B-Komplex", "single_vitamin_b_komplex", "Vitamin B-Komplex")
        vitamin_d, vitamin_d_weeks, vitamin_d_freq = infusion_row("Vit. D", "single_vitamin_d", "Vitamin D")
        vitamin_b6_b12_folsaeure, vitamin_b6_b12_folsaeure_weeks, vitamin_b6_b12_folsaeure_freq = infusion_row("Vit. B6/B12/Folsäure", "single_vitamin_b6_b12_folsaeure", "Vitamin B6, B12 und Folsäure")
        vitamin_b3, vitamin_b3_weeks, vitamin_b3_freq = infusion_row("Vit. B3", "single_vitamin_b3", "Vitamin B3")
        ...
        zusaetze = st.multiselect(
            ...
            key="zusaetze_select", placeholder="Zusätze auswählen..."
        )

        st.session_state.infusion_data = {
            "revita_immune": revita_immune, "revita_immune_weeks": revita_immune_weeks, "revita_immune_freq": revita_immune_freq,
            "revita_immune_plus": revita_immune_plus, "revita_immune_plus_weeks": revita_immune_plus_weeks, "revita_immune_plus_freq": revita_immune_plus_freq,
            "revita_heal": revita_heal, "revita_heal_weeks": revita_heal_weeks, "revita_heal_freq": revita_heal_freq,
            "revita_bludder": revita_bludder, "revita_bludder_weeks": revita_bludder_weeks, "revita_bludder_freq": revita_bludder_freq,
            "revita_ferro": revita_ferro, "revita_ferro_weeks": revita_ferro_weeks, "revita_ferro_freq": revita_ferro_freq,
            "revita_energy": revita_energy, "revita_energy_weeks": revita_energy_weeks, "revita_energy_freq": revita_energy_freq,
            "revita_focus": revita_focus, "revita_focus_weeks": revita_focus_weeks, "revita_focus_freq": revita_focus_freq,
            "revita_nad": revita_nad, "revita_nad_weeks": revita_nad_weeks, "revita_nad_freq": revita_nad_freq,
            "revita_relax": revita_relax, "revita_relax_weeks": revita_relax_weeks, "revita_relax_freq": revita_relax_freq,
            "revita_fit": revita_fit, "revita_fit_weeks": revita_fit_weeks, "revita_fit_freq": revita_fit_freq,
            "revita_hangover": revita_hangover, "revita_hangover_weeks": revita_hangover_weeks, "revita_hangover_freq": revita_hangover_freq,
            "revita_beauty": revita_beauty, "revita_beauty_weeks": revita_beauty_weeks, "revita_beauty_freq": revita_beauty_freq,
            "revita_antiaging": revita_antiaging, "revita_antiaging_weeks": revita_antiaging_weeks, "revita_antiaging_freq": revita_antiaging_freq,
            "revita_detox": revita_detox, "revita_detox_weeks": revita_detox_weeks, "revita_detox_freq": revita_detox_freq,
            "revita_chelate": revita_chelate, "revita_chelate_weeks": revita_chelate_weeks, "revita_chelate_freq": revita_chelate_freq,
            "revita_liver": revita_liver, "revita_liver_weeks": revita_liver_weeks, "revita_liver_freq": revita_liver_freq,
            "revita_leakygut": revita_leakygut, "revita_leakygut_weeks": revita_leakygut_weeks, "revita_leakygut_freq": revita_leakygut_freq,
            "revita_infection": revita_infection, "revita_infection_weeks": revita_infection_weeks, "revita_infection_freq": revita_infection_freq,
            "revita_joint": revita_joint, "revita_joint_weeks": revita_joint_weeks, "revita_joint_freq": revita_joint_freq,
            "mito_energy": mito_energy, "mito_energy_weeks": mito_energy_weeks, "mito_energy_freq": mito_energy_freq,
            "schwermetalltest": schwermetalltest, "schwermetalltest_weeks": schwermetalltest_weeks, "schwermetalltest_freq": schwermetalltest_freq,
            "procain_basen": procain_basen, "procain_basen_weeks": procain_basen_weeks, "procain_basen_freq": procain_basen_freq,
            "procain_2percent": procain_2percent,
            "artemisinin": artemisinin, "artemisinin_weeks": artemisinin_weeks, "artemisinin_freq": artemisinin_freq,
            "perioperative": perioperative, "perioperative_weeks": perioperative_weeks, "perioperative_freq": perioperative_freq,
            "detox_standard": detox_standard, "detox_standard_weeks": detox_standard_weeks, "detox_standard_freq": detox_standard_freq,
            "detox_maxi": detox_maxi, "detox_maxi_weeks": detox_maxi_weeks, "detox_maxi_freq": detox_maxi_freq,
            "aufbauinfusion": aufbauinfusion, "aufbauinfusion_weeks": aufbauinfusion_weeks, "aufbauinfusion_freq": aufbauinfusion_freq,
            "oxyvenierung": oxyvenierung, "oxyvenierung_weeks": oxyvenierung_weeks, "oxyvenierung_freq": oxyvenierung_freq,
            "anti_aging": anti_aging, "anti_aging_weeks": anti_aging_weeks, "anti_aging_freq": anti_aging_freq,
            "nerven_aufbau": nerven_aufbau, "nerven_aufbau_weeks": nerven_aufbau_weeks, "nerven_aufbau_freq": nerven_aufbau_freq,
            "leberentgiftung": leberentgiftung, "leberentgiftung_weeks": leberentgiftung_weeks, "leberentgiftung_freq": leberentgiftung_freq,
            "anti_oxidantien": anti_oxidantien, "anti_oxidantien_weeks": anti_oxidantien_weeks, "anti_oxidantien_freq": anti_oxidantien_freq,
            "aminoinfusion": aminoinfusion, "aminoinfusion_weeks": aminoinfusion_weeks, "aminoinfusion_freq": aminoinfusion_freq,
            "relax_infusion": relax_infusion, "relax_infusion_weeks": relax_infusion_weeks, "relax_infusion_freq": relax_infusion_freq,
            ...
            "vitamin_c": vitamin_c, "vitamin_c_weeks": vitamin_c_weeks, "vitamin_c_freq": vitamin_c_freq,
            "vitamin_b_komplex": vitamin_b_komplex, "vitamin_b_komplex_weeks": vitamin_b_komplex_weeks, "vitamin_b_komplex_freq": vitamin_b_komplex_freq,
            "vitamin_d": vitamin_d, "vitamin_d_weeks": vitamin_d_weeks, "vitamin_d_freq": vitamin_d_freq,
            "vitamin_b6_b12_folsaeure": vitamin_b6_b12_folsaeure, "vitamin_b6_b12_folsaeure_weeks": vitamin_b6_b12_folsaeure_weeks, "vitamin_b6_b12_folsaeure_freq": vitamin_b6_b12_folsaeure_freq,
            "vitamin_b3": vitamin_b3, "vitamin_b3_weeks": vitamin_b3_weeks, "vitamin_b3_freq": vitamin_b3_freq,
            "zusaetze": zusaetze,
        }
